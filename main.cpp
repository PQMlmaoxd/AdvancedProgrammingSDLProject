...
int main(int argc, char* argv[]) {
    ...
    // Chọn New Game hoặc Load Game
    bool newGame = menu.selectGameMode();
    int saveSlot = menu.selectSaveSlot();
    std::string mazeFile = "Save/Save" + std::to_string(saveSlot) + "_maze.txt";
    std::string playerFile = "Save/Save" + std::to_string(saveSlot) + "_player.txt";
    
    createSaveDirectory(); // Tạo thư mục Save nếu chưa tồn tại
    
    Maze maze;
    std::ifstream mazeCheck(mazeFile);
    if (!newGame && mazeCheck.good()) {
